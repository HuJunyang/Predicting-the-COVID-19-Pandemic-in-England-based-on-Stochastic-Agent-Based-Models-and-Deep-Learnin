# -*- coding: utf-8 -*-
"""
Created on Tue Aug 17 10:46:41 2021
...
to_plot = sc.objdict({
    ...
    #'Incidence': ['incidence'],
    'R ratio': ['r_eff'],
    'cumulative Vaccinated': ['cum_vaccinated'],
})
...
# Filepaths
data_path = 'D:\dt\covasim\covasim_code\England_Covid_cases_feb28.xlsx' # England_Covid_cases_august10
resfolder = 'results_Vac'

# Important dates
start_day = '2020-01-31'
end_day = '2021-02-28'
data_end = '2020-12-03' # Final date for calibration
...
vx_duration = 21 # Number of days of vaccine rollout
...
vx_rollout = {
    ...
    60: dict(start_age=60, end_age=75,  start_day='2021-01-22'),
...
def subtarget(sim, age=None, vx_scenario=None):
    ...
    return {'inds': inds, 'vals': coverage*np.ones(len(inds))} #0.65  0.4 or 0.7
...
def make_sim(seed, beta, calibration=True, future_symp_test=None, scenario=None, vx_scenario=None, end_day='2021-10-30', verbose=0):
    ...
    pars = sc.objdict(
        ...
        verbose      = verbose,
        rel_symp_prob = 1, 
        rel_severe_prob = 0.78,#0.8
        rel_crit_prob = 3.5,# how to interpret this factor? 2.3
        rel_death_prob= 1.3, #1.3
    )
   ...
   # ADD BETA INTERVENTIONS
    sbv = 0.50 #sbv stands for what? why the scles for the school remain at sbv for some of the periods?
    beta_past  = sc.odict({'2020-02-14': [1.00, 1.00, 0.95, 0.95],# 1
                           '2020-03-16': [1.00, 0.90, 0.90, 0.90], #
                           '2020-03-23': [1.00, 0.02, 0.20, 0.65], ####
                           '2020-03-30': [1.00, 0.02, 0.20, 0.45],
                           '2020-04-06': [1.00, 0.02, 0.20, 0.25],
                           '2020-06-01': [1.00, 0.23, 0.40, 0.40],# 3
                           '2020-06-15': [1.00, 0.38, 0.50, 0.50],
                           '2020-07-22': [1.15, 0.00, 0.40, 0.50], ######
                           '2020-09-02': [1.15, sbv,  0.60, 0.70],
                           '2020-10-01': [1.15, sbv, 0.50, 0.60], #
                           '2020-10-16': [1.15, sbv, 0.40, 0.50], #
                           '2020-10-26': [1.15, 0.00, 0.30, 0.40], #? autumn break
                           '2020-11-05': [1.15, sbv, 0.20, 0.30], ####
                           '2020-11-14': [1.15, sbv, 0.20, 0.25],
                           '2020-11-21': [1.15, sbv, 0.20, 0.20],
                           '2020-11-30': [1.15, sbv, 0.20, 0.20], ####
                           
                           
                            ####scenario 1 full reopen in December + full lockdown in Jan
                           # '2020-12-04': [1.15, sbv, 0.50, 0.80],
                           # '2020-12-10': [1.50, 0.00, 0.50, 0.80],
                           # '2020-12-17': [1.50, 0.00, 0.50, 0.80],
                           # '2020-12-24': [1.50, 0.00, 0.00, 0.80],
                           # '2020-12-26': [1.50, 0.00, 0.20, 0.80],
                           # '2020-12-31': [1.50, 0.00, 0.20, 0.80],
                           
                           #  '2021-01-01': [1.50, 0.00, 0.20, 0.30],
                           #  '2021-01-04': [1.10, 0.14, 0.20, 0.30], # 5
                           ...
                           #      # ####scenario 2 full reopen in December + partial lockdown 
                           # '2020-12-04': [1.15, sbv, 0.50, 0.80],
                           #  '2020-12-10': [1.50, 0.00, 0.50, 0.80],
                           #  '2020-12-17': [1.50, 0.00, 0.50, 0.80],
                           #  '2020-12-24': [1.50, 0.00, 0.00, 0.80],
                           ...
                           #   '2021-01-01': [1.50, 0.00, 0.40, 0.50],
                           #   '2021-01-04': [1.10, sbv, 0.40, 0.50], # 5
                           #   '2021-01-11': [1.05, sbv, 0.40, 0.50],
                           #   '2021-01-18': [1.05, sbv, 0.40, 0.60],
                           #   '2021-01-30': [1.05, sbv, 0.40, 0.60],
                           #   '2021-02-08': [1.05, sbv, 0.40, 0.60],

                           
                           # # # ####scenario 3 phased reopen in December + partial lockdown 
                              '2020-12-04': [1.15, sbv, 0.40, 0.40],
                            '2020-12-10': [1.50, 0.00, 0.50, 0.50],
                            '2020-12-17': [1.50, 0.00, 0.50, 0.70],
                            '2020-12-25': [1.50, 0.00, 0.00, 0.80],
                            '2020-12-26': [1.50, 0.00, 0.20, 0.80],
                            '2020-12-31': [1.50, 0.00, 0.20, 0.80],
    ...
    # Add B.1.117 strain
    b117 = cv.variant('b117', days=np.arange(sim.day('2020-08-20'), sim.day('2020-08-30')), n_imports=470)#
    b117.p['rel_beta']        = 1.60
    b117.p['rel_symp_prob']   = 1.0
    ...
    tti_day_april21= sim.day('2021-04-01')
    ...
    # tn = 0.09
    s_prob_sep = 0.05769 # higher#
    s_prob_oct = 0.07769 # 0.07
    s_prob_nov = 0.08769 #
    s_prob_dec = 0.06769 # lower
    s_prob_jan = 0.08769 ####
    
    
    s_prob_march = 0.07769 #
    s_prob_april21 = 0.07769 #
    ...
    interventions += [
        ...
        cv.test_prob(symp_prob=s_prob_march, asymp_prob=0.008, symp_quar_prob=0.0, start_day=tti_day_march, end_day=tti_day_april21-1, test_delay=t_delay),
        cv.test_prob(symp_prob=s_prob_april21, asymp_prob=0.008, symp_quar_prob=0.0, start_day=tti_day_april21, end_day=tti_day_june21-1, test_delay=t_delay),
        ...
        cv.dynamic_pars({'iso_factor': {'days': tti_day_jan, 'vals': iso_vals6}}),
    ...
    ###scenario optimistic 
    dose_pars['nab_init']= dict(dist='normal', par1=4, par2=2)
    dose_pars['nab_boost'] = 4
    
    
    ##scenario pessimistic 
    # dose_pars['nab_init']= dict(dist='normal', par1=0.25, par2=2)
    # dose_pars['nab_boost'] = 2
...
if __name__ == '__main__':
    ...
    if whattorun=='quickfit':
        s0 = make_sim(seed=1, beta=0.0079, end_day='2021-02-04', verbose=0.1, vx_scenario=0)
        sims = []
        ...
        if save_sim:
                    msim.reduce()
                    #msim.reduce(quantiles=[0.25, 0.75])
                    msim.save(f'{resfolder}/eng.a.obj',keep_people=True)
        if do_plot:
            msim.plot(to_plot=to_plot, do_save=True, do_show=False, fig_path='England_calibration.png',
                      legend_args={'loc': 'upper left'}, axis_args={'hspace': 0.2,'wspace':0.2}, interval=50, n_cols=3,
                      fig_args=dict(figsize=(30,15)))
